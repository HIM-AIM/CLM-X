...
def infer_classes_from_h5ad(h5ad_path: str, column: str = "cell_type"):
    adata = sc.read(h5ad_path)
    if column not in adata.obs.columns:
        raise ValueError(f"Column '{column}' not found in {h5ad_path}.")

    classes = adata.obs[column].unique().tolist()
    try:
...
def main(_config):
    ...
    if args.model_task == "finetune":
        callbacks.insert(
            ...
            ModelCheckpoint(
                ...
                filename=f"{args.exp_name}-epoch{{epoch:02d}}",
                monitor="val_acc_f1_sum",
                mode="max",
                save_top_k=1,
                save_last=False,
    ...
    if args.task != "rnaatacmlm":
        raise ValueError("annotation script expects args.task == 'rnaatacmlm'.")

    logger.info("Building dataloaders...")
    train_loader, val_loader, test_loader, gene_tokens = (
        build_rna_atac_loaders_presplit_train_test(args, logger=logger)
    )
    ...
    # Class metadata for annotation head
    cell_type_col = str("cell_type")
    classes, label2id = infer_classes_from_h5ad(args.rna_test_h5ad, column=cell_type_col)
    args.num_classes = len(classes)
    args.cell_types = classes
    args.label2id = label2id

    logger.info(f"num_classes={args.num_classes}, cell_type_column={cell_type_col}")
